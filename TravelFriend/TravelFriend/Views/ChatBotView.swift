    ...
    var body: some View {
        ZStack{
            VStack {
                ...
                // 버튼 선택하면 사라져야함
                if isButtonShowing {
                    LocationSugestionView(location: $location, isButtonShowing: $isButtonShowing)
                }
                ...
                //.clipShape(.rect(cornerRadius: 5))
                    .onChange(of: location) { _, newValue in
                        Task {
                            ...
                            do {
                                let response = try await requestGPT4(prompt: "\(location)에서 사용하는 \(self.country)어 회화 표현을 알려줘. 첫 줄에는 한국어 의미, 둘째 줄에는 \(location)어 표기, 셋째 줄에는 \(location)어의 한국어 발음 표기로 제시해주고 총 15개 이상 표현을 제시해줘")
                                DispatchQueue.main.async {
        ...
        VStack {
            LazyVGrid(columns: columns) {
                ForEach(self.locationList, id: \.self) { location in
                    LocationButton(location: $location, isButtonShowing: $isButtonShowing, buttonTitle: location)
                }
            }
        }
    }
